...
$profileSql = "SELECT id, name, email, profile_picture, cover_photo, bio, location, website, created_at, status 
...
// Count friends for sidebar (current user's friends)
...
// Count profile user's friends for display
$profileFriendSql = "SELECT COUNT(*) as friend_count FROM friends WHERE (user_id = ? OR friend_id = ?) AND status = 'approved'";
$profileFriendStmt = $conn->prepare($profileFriendSql);
$profileFriendStmt->bind_param("ii", $profile_id, $profile_id);
$profileFriendStmt->execute();
$profile_friend_count = $profileFriendStmt->get_result()->fetch_assoc()['friend_count'];
$profileFriendStmt->close();

// Fetch pending friend requests for sidebar (current user's)
...
// Count user posts for sidebar (current user's)
...
// Count profile user's posts
$profilePostCountSql = "SELECT COUNT(*) as post_count FROM posts WHERE user_id = ?";
$profilePostCountStmt = $conn->prepare($profilePostCountSql);
$profilePostCountStmt->bind_param("i", $profile_id);
$profilePostCountStmt->execute();
$profile_post_count = $profilePostCountStmt->get_result()->fetch_assoc()['post_count'];
$profilePostCountStmt->close();

// Fetch profile user's posts (only if friends or own profile)
$profile_posts = [];
if ($is_own_profile || $friendship_status === 'approved') {
    $postsSql = "SELECT p.*, 
    ...
    $postsStmt->bind_param("ii", $current_user_id, $profile_id);
    $postsStmt->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($profile_user['name']); ?>'s Profile - Manexis</title>
        ...
        <main class="main-content">
                    ...
                    <div class="profile-avatar-section">
                        ...
                        <div class="profile-details">
                            ...
                            <div class="profile-meta">
                                ...
                                <?php endif; ?>
                                <div class="profile-meta-item">👥 <?php echo $profile_friend_count . ' Friends'; ?></div>
                                <div class="profile-meta-item">📝 <?php echo $profile_post_count . ' Posts'; ?></div>
                            </div>
            ...
            <div id="about" class="tab-content active">
                <div class="form-section">
                    <h3>About <?php echo htmlspecialchars($profile_user['name']); ?></h3>
                    <div class="info-grid">
                        <div class="info-item">
                            <div class="info-label">Full Name</div>
                            <div class="info-value"><?php echo htmlspecialchars($profile_user['name']); ?></div>
                        </div>
                        <?php if (!empty($profile_user['location'])): ?>
                        <div class="info-item">
                            <div class="info-label">Location</div>
                            <div class="info-value"><?php echo htmlspecialchars($profile_user['location']); ?></div>
                        </div>
                        <?php endif; ?>
                        <?php if (!empty($profile_user['website'])): ?>
                        <div class="info-item">
                            <div class="info-label">Website</div>
                            <div class="info-value"><a href="<?php echo htmlspecialchars($profile_user['website']); ?>" target="_blank"><?php echo htmlspecialchars($profile_user['website']); ?></a></div>
                        </div>
                        <?php endif; ?>
                        <div class="info-item">
                            <div class="info-label">Member Since</div>
                            <div class="info-value"><?php echo date('F j, Y', strtotime($profile_user['created_at'])); ?></div>
                        </div>
                        <div class="info-item">
            ...
            <!-- Posts Tab (Only visible to friends or self) -->
            <?php if ($is_own_profile || $friendship_status === 'approved'): ?>
                ...
                <div class="form-section">
                    ...
                    <?php if (empty($profile_posts)): ?>
                        <div class="empty-state">
                            ...
                            <h3>No posts yet</h3>
                            <p><?php echo $is_own_profile ? 'You haven\'t created any posts yet.' : htmlspecialchars($profile_user['name']) . ' hasn\'t created any posts yet.'; ?></p>
                            <?php if ($is_own_profile): ?>
                            ...
                            <?php foreach ($profile_posts as $post): ?>
                                <div class="post-card">
                                    <div class="post-header">
                                        <div class="post-author-avatar" style="<?php echo !empty($profile_user['profile_picture']) ? 'background-image: url(../../uploads/profile_pictures/' . htmlspecialchars($profile_user['profile_picture']) . ');' : ''; ?>">
                                            <?php if (empty($profile_user['profile_picture'])) echo strtoupper(substr($profile_user['name'] ?? 'U', 0, 1)); ?>
                                        </div>
                                        <div class="post-author-info">
                                            <div class="post-author-name"><?php echo htmlspecialchars($profile_user['name']); ?></div>
                                    ...
                                    <?php if (!empty($post['image_filename'])): ?>
                                        <div class="post-image-full">
                                            ...
                                            <?php if (!empty($post['image_caption'])): ?>
                                                <div class="image-caption-full" style="margin-top: 8px; color: #718096; font-size: 14px;">
                    ...
                    <div class="form-section">
                        ...
                        <div class="info-grid">
                            <div class="info-item">
                                ...
                                <div class="info-value">#<?php echo htmlspecialchars($profile_user['id'] ?? 'N/A'); ?></div>
                            </div>
                            <div class="info-item">
                                <div class="info-label">Account Status</div>
                                <div class="info-value">
                            ...
                            <div class="info-item">
                                ...
                                <div class="info-value">
                                    <?php if ($is_own_profile): ?>
                                        <span style="color: #276749;">Your Profile</span>
                                    <?php elseif ($friendship_status === 'approved'): ?>
                                        <span style="color: #276749;">Friends</span>
                                    <?php elseif ($friendship_status === 'pending'): ?>
                                        <span style="color: #d69e2e;">Request Pending</span>
                                    <?php else: ?>
                                        <span style="color: #c53030;">Not Connected</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="info-grid">
                            <div class="info-item">
                                ...
                                <div class="info-value" style="font-size: 24px; color: #3498db;"><?php echo $profile_friend_count; ?></div>
                            </div>
                            <div class="info-item">
                                <div class="info-label">Total Posts</div>
                                <div class="info-value" style="font-size: 24px; color: #3498db;"><?php echo $profile_post_count; ?></div>
                            </div>
                            <div class="info-item">
                                <div class="info-label">Account Age</div>
                                <div class="info-value" style="font-size: 24px; color: #3498db;">
                                    <?php 
                                    $accountAge = floor((time() - strtotime($profile_user['created_at'])) / (60 * 60 * 24));
        ...
        <aside class="right-sidebar">
            ...
            <div class="sidebar-card">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <div class="quick-stats-item">
                        ...
                        <span style="font-weight: 600; color: #2d3748;"><?php echo $profile_friend_count; ?></span>
                    </div>
                    <div class="quick-stats-item">
                        <span style="color: #718096; font-size: 14px;">📝 Posts</span>
                        <span style="font-weight: 600; color: #2d3748;"><?php echo $profile_post_count; ?></span>
                    </div>
                    <div class="quick-stats-item">
                    ...
                    <div class="quick-stats-item">
                        ...
                        <span style="font-weight: 600; color: #2d3748;"><?php echo date('M Y', strtotime($profile_user['created_at'])); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="sidebar-card">
                ...
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ...
                    <form method="POST" action="add_friend.php" style="margin: 0;">
                        ...
                        <input type="hidden" name="send_request" value="1">
                        <input type="hidden" name="friend_id" value="<?php echo $profile_user['id']; ?>">
                        <button type="submit" style="display: flex; align-items: center; gap: 10px; padding: 10px; background: #3498db; color: white; border: none; border-radius: 8px; text-decoration: none; transition: background 0.2s; width: 100%; cursor: pointer;">
                            <span>👋</span>
            ...
            <div class="sidebar-card">
                ...
                <div style="font-size: 13px; color: #718096; line-height: 1.6;">
                    <?php if ($is_own_profile): ?>
                        <p style="margin: 0 0 10px 0; color: #276749;">👁️ This is your public profile</p>
                        <p style="margin: 0;">Other users see this view when visiting your profile.</p>
                    <?php elseif ($friendship_status === 'approved'): ?>
                        <p style="margin: 0 0 10px 0; color: #276749;">✅ You are friends</p>
                        <p style="margin: 0;">You can see each other's posts and activities.</p>
                    <?php elseif ($friendship_status === 'pending'): ?>
                        <p style="margin: 0 0 10px 0; color: #d69e2e;">⏳ Friend request sent</p>
                        <p style="margin: 0;">Waiting for <?php echo htmlspecialchars($profile_user['name']); ?> to accept your request.</p>
                    <?php else: ?>
                        <p style="margin: 0 0 10px 0; color: #c53030;">🔒 Not connected</p>
                        <p style="margin: 0;">Send a friend request to see more content.</p>
                    <?php endif; ?>
                </div>
            </div>
        </aside>
